...
public abstract class ProdutorConsumidorMultiThread<T> {
			...
			Thread t = new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						int running = threadsRunning.decrementAndGet();
						LOGGER.debug("Resta(m) " + running + " thread(s) neste objeto");
					}
				}
			});
			
			if (nameForDebug != null) {
				t.setName(nameForDebug + " (" + (i+1) + ")");
			}
	 ...
	 */
	public void aguardarTermino() throws RuntimeException, InterruptedException {
		isRunning = false;
