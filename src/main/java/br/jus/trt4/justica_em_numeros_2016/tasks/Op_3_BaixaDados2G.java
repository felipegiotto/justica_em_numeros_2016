...
public class Op_3_BaixaDados2G {
	...
	public static void main(String[] args) throws SQLException, Exception {
		...
		// Conexão com o banco de dados do PJe
		try (Connection conexaoBasePrincipal = Auxiliar.getConexaoPJe2G()) {
			
			// Objetos auxiliares para gerar o XML
			ObjectFactory factory = new ObjectFactory();
			Processos processos = factory.createProcessos();
			
			// Le o SQL que fará a consulta de todos os processos
			...
			try (NamedParameterStatement ps = new NamedParameterStatement(conexaoBasePrincipal, sqlConsultaProcessos)) {
				ps.setString("cd_municipio_ibge_trt", Auxiliar.getParametroConfiguracao("codigo_municipio_ibge_trt", true));
				...
				try (ResultSet rs = ps.executeQuery()) {
					int i=0;
					while (rs.next()) {
						String numeroCompletoProcesso = rs.getString("numero_completo_processo");
						LOGGER.debug("Analisando processo " + numeroCompletoProcesso + " (" + (++i) + ")");
						
						TipoProcessoJudicial processoJudicial = new TipoProcessoJudicial();
						preencheProcessoJudicialComBaseEmScriptDoTRT14(processoJudicial, rs, conexaoBasePrincipal);
			...
			File arquivoSaida = new File("output/dados_2g.xml");
	 ...
	 * @throws IOException 
	 */
	private static void preencheProcessoJudicialComBaseEmScriptDoTRT14(TipoProcessoJudicial processoJudicial, ResultSet rsProcesso, Connection conexaoBasePrincipal) throws SQLException, IOException {
		...
		// Le o SQL que fará a consulta de todos os polos
		String sqlConsultaPolos = Auxiliar.lerConteudoDeArquivo("src/main/resources/baixa_dados_2g/02_consulta_polos.sql");
		try (NamedParameterStatement nsPolos = new NamedParameterStatement(conexaoBasePrincipal, sqlConsultaPolos)) {
			...
			try (ResultSet rsPolos = nsPolos.executeQuery()) {
				while (rsPolos.next()) {
					...
					// Le o SQL que fará a consulta das partes
					String sqlConsultaPartes = Auxiliar.lerConteudoDeArquivo("src/main/resources/baixa_dados_2g/03_consulta_partes.sql");
					try (NamedParameterStatement nsPartes = new NamedParameterStatement(conexaoBasePrincipal, sqlConsultaPartes)) {
						...
						try (ResultSet rsPartes = nsPartes.executeQuery()) {
							while (rsPartes.next()) {
								...
								// Le o SQL que fará a consulta dos documentos da pessoa
								...
								try (NamedParameterStatement nsDocumentos = new NamedParameterStatement(conexaoBasePrincipal, sqlConsultaDocumentos)) {
		...
		// Le o SQL que fará a consulta dos assuntos do processo
		String sqlConsultaAssuntos = Auxiliar.lerConteudoDeArquivo("src/main/resources/baixa_dados_2g/05_consulta_assuntos.sql");
		try (NamedParameterStatement nsAssuntos = new NamedParameterStatement(conexaoBasePrincipal, sqlConsultaAssuntos)) {
...
//		orgaoJulgador.setNomeOrgao(rsProcesso.getString("ds_orgao_julgador")); // TODO: Falta definir origem do campo "NomeOrgao"!
		...
		// Le o SQL que fará a consulta dos movimentos processuais
		String sqlConsultaMovimentos = Auxiliar.lerConteudoDeArquivo("src/main/resources/baixa_dados_2g/06_consulta_movimentos.sql");
		try (NamedParameterStatement nsMovimentos = new NamedParameterStatement(conexaoBasePrincipal, sqlConsultaMovimentos)) {
			...
			try (ResultSet rsMovimentos = nsMovimentos.executeQuery()) {
				while (rsMovimentos.next()) {
					...
					try (NamedParameterStatement nsComplementos = new NamedParameterStatement(conexaoBasePrincipal, sqlConsultaComplementos)) {
